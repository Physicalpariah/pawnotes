...
public class AppData: ObservableObject {
    ...
    @Published var topPadding = 10.0
    @Published var horizPadding = 10.0
    ...
    func Initialise() {
        ...
        if count > 0 {
            for number in 1...count {
                ...
                var d: PKDrawing
                do {
                    try d = PKDrawing.init(data: LoadPageData(title: page.title))
                }
                catch{
                    print("hoo boy here we go.", number)
                    count = number;
                    return
                }
            }
    ...
    public func GetDisplayForPage(item : ViewData, scale:CGFloat) -> UIImage {
        var d: PKDrawing
        do {
            try d = PKDrawing.init(data: LoadPageData(title: item.title))
            d.scale(scaleFactor: scale)
            return d.image(from: CGRect(x: 0, y: 0, width: 120, height: 120), scale: 1);
        } catch {
            //            throw("Error loading drawing object")
    ...
    public func Clear(viewData: ViewData){
        let defaults = UserDefaults.standard
        defaults.removeObject(forKey: viewData.title);
    }
    ...
    public func LoadPageData(title:String) -> Data
    {
        let defaults = UserDefaults.standard
        return defaults.object(forKey: sessionName + title) as? Data ?? Data()
    }
